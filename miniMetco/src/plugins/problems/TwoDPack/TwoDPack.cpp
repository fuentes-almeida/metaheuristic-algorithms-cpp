...
//Borra los elementos est�ticos cuando el �ltimo individuo es liberado
TwoDPack::~TwoDPack(){
	nInds--;
	if (nInds == 0){
		endLocalSearch1();
...
bool TwoDPack::init (const vector<string> &params){
	...
	//Inicializaciones para las b�squedas locales
	initLocalSearch1();
	initLocalSearch2();

	setNumberOfObj(1); //un �nico objetivo
...
//Generaci�n de aleatorio y evaluaci�n de objetivo
...
// Actualizar el fitness y aparici�n de parejas considerando la casilla (x1, y1) - (x2, y2)
void TwoDPack::update(const int x1, const int y1, const int x2, const int y2){
	...
	if (appearance[n1][n2] == 1){
		setObj(0, objective + twoDPackingProblem->getValue(n1, n2));
		objective += twoDPackingProblem->getValue(n1, n2);
	}
	if (n1 != n2)
		appearance[n2][n1]++;
}
...
// Ejecutar una b�squeda local sobre el elemento actual
...
//- Los vecinos consisten en cambiar una �nica casilla, por el valor que maximice la asignacion
//- Se mueve al primer vecino que encuentre que la asignacion mejora
//- Se para cuando no se puede cambiar una �nico asignaci�n consiguiendo que el valor mejore
...
//La busqueda local tiene las siguientes caracteristicas:
//- Los vecinos consisten en cambiar 2 casillas adyacentes, por el valor que maximice la asignaci�n
//- Como las posibles asignaciones son demasiados (400 x 400 en la instancia del concurso) se ha
//dise�ado un m�todo basado en cotas superiores, para no tener que visitar todas las posibles
...
// Suprimir una asignaci�n, restando el fitness y la apareici�n de las parejas correspondientes
int TwoDPack::removeAssignation(const int x, const int y){
	...
	setGrid(x, y, -1);
	setObj(0, objective - removeFitness);
	objective -= removeFitness;
	return removeFitness;
}

// Obtener el fitness asociado a realizar una asignaci�n
...
int TwoDPack::getAssignationCost(const int x1, const int y1, const int v1, const int x2, const int y2, const int v2){
	...
	//a�adimos la pareja value1 - value2
	int menor, mayor;
	if (v1 < v2){
		menor = v1;
		mayor = v2;
...
// Realizar la asignaci�n de un valor a una casilla
	...
	for (int i = initX; i <= finX; i++){
		for (int j = initY; j <= finY; j++){
			...
			if (appearance[value][value2] == 1){
				setObj(0, objective + twoDPackingProblem->getValue(value, value2));
				objective += twoDPackingProblem->getValue(value, value2);
			}
		}
	}
	setGrid(x, y, value);
...
// Usado por localSearch1, busca la mejor asignaci�n posible para la casilla (x,y)
bool TwoDPack::improvePosition(const int x, const int y){
	...
	for (int i = initX; i <= finX; i++){
		for (int j = initY; j <= finY; j++){
			if ((i == x) && (y == j))//con los vecinos, excluy�ndose a si mismo
				continue;
			// s�lo consideramos valores que pueden puntuar
...
bool TwoDPack::improvePosition(const Position &p1, const Position &p2, const Position &p3){
	...
	if (objective != (originalFitness - valuep1p2p3 + actualValue)){
		cout << "Fail: " << objective << " " << originalFitness - valuep1p2p3 + actualValue << endl;
		exit(-1);
	}
	return (actualValue > valuep1p2p3);
}
// Busca la mejor asignaci�n posible para las casillas p1 y p2 (a la vez)
bool TwoDPack::improvePosition(const Position &p1, const Position &p2){
	...
	if (objective != (originalFitness - valuep1p2 + actualValue)){
		cout << "Fail: " << objective << " " << originalFitness - valuep1p2 + actualValue << endl;
		exit(-1);
	}
...
// Mutaci�n uniforme: se elige un valor de probabilidad de mutacion mutProb entre mutProbLow y mutProbHigh
...
void TwoDPack::mutationUniformFixedSize(double pm){
	int size = twoDPackingProblem->getXSize() * twoDPackingProblem->getYSize() * pm;
	...
	random_shuffle(considered.begin(), considered.end());

	for (int i = 0; i < size; i++){
		int first = considered[i].first;
		int second = considered[i].second;
		removeAssignation(first, second);
		makeAssignation(first, second, (int) ((double)(twoDPackingProblem->getMaxNumber()) * rand() / (RAND_MAX + 1.0)));
	}
}
...
// Mutaci�n basada en ventanas: se divide la grid en ventanas y
// se muta con mayor probabilidad aquellos elementos que est�n dentro de la peor ventana
...
double TwoDPack::getDistance(Individual &ind2) {
	int actualDist = 0;
