...
void Individual::dependentMutation (const double pm){
	cout << "Mutation internal specified, but dependentMutation method not reimplemented" << endl;
	exit(-1);
}

void Individual::dependentCrossover(Individual *ind){
	cout << "Croosver internal specified, but dependentCrossover method not reimplemented" << endl;
	exit(-1);
}

// Change binary-flip (NUEVO) 
void Individual::mutate_binary_flip(const double pm, int origin, int destination){
...
void Individual::mutate_uniform_one (double pm) {
	...
	if (vmut < pm) {
		...
		var[indElem] = var[indElem] * (getMaximum(indElem) - getMinimum(indElem)) + getMinimum(indElem); 
	}
}
...
// Sacado del software de Málaga: DEME
// NUEVO
void Individual::mutate_Pol (double pm, int origin, int destination) {
	destination = (destination == -1)?var.size():destination;
	const double n = 20;			//parámetro del mutation (distribution index)
...
void Individual::crossover_SBX (Individual *ind, int origin, int destination) {
	...
	for (unsigned int i = origin; i < destination; i++){
		...
		if (cross >= 0.5){
			u = ((double)rand())/(RAND_MAX + 1.0);//genera entre 0 y 1 (no incluid9
...
void Individual::crossover_range (Individual *ind, const int points) {
	...
	for (int i = 0; i < points; i++)
		index.push_back((int) ((double)(var.size()-1)*rand()/(RAND_MAX+1.0)));
...
// NUEVO
void Individual::crossover_uniform (Individual *ind, int origin, int destination){
	destination = (destination == -1)?var.size():destination;
	for (unsigned int i = origin; i < destination; i++){
		if (rand() > (RAND_MAX / 2)){
...
// Se parten ambos individuos por el mismo punto de corte, generado éste
...
void Individual::crossover_OPC(Individual* ind, int origin, int destination) {
	...
	int size = destination - origin;
        int vcross = int((double)(origin + size * rand())/(RAND_MAX + 1.0)); // Valor [origin-destination]
...
void Individual::crossover_TPC(Individual* ind, int origin, int destination){
	destination = (destination == -1)?var.size():destination;
	int size = destination - origin;
        int vcross1 = int((double)(origin + size * rand())/(RAND_MAX + 1.0)); // Valor [origin-destination]
	int vcross2 = int((double)(origin + size * rand())/(RAND_MAX + 1.0)); // Valor [origin-destination]
...
// Crea un nuevo individuo generando valores aleatorios para las variables del mismo
...
Individual *Individual::deserialize(const int nVar, const int nObj, const int nMig, const long double *buffer, int &count){
	Individual *nuevo = clone();
	count = 0;
	...
	for (int i = 0; i < nVar; i++)
		nuevo->var[i] = buffer[count++];

	// Objetivos
	nuevo->setNumberOfObj(nObj);
	for (int i = 0; i < nObj; i++)
		nuevo->obj[i] = buffer[count++];

	// Otros parámetros a migrar
	nuevo->setMigrationDataSize(nMig);
	for (int i = 0; i < nMig; i++)
		nuevo->migrationData[i] = buffer[count++];
...
double Individual::getDistance(Individual &ind2){//calculate euclidean distance
	...
	for (int i = 0; i < getNumberOfVar(); i++){
		double dvar= getVar(i) - ind2.getVar(i);
		distance += (dvar * dvar);
	}
...
Individual *getInd(const string &pluginPath, const string &name, const vector<string> &indArgs, bool printError){
	return ((Individual *)(loadPlugin("Problem", pluginPath, name, indArgs, printError, true)));
}
