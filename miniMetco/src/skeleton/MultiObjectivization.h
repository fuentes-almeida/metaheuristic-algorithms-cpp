...
class MultiObjectivization : public Plugin {
	public:
		virtual void convertToMultiObjective(vector<Individual *> *parents, vector<Individual *> *offSprings, vector<Individual *> *reference, int ordObj, int numGeneration, double completedRatio) = 0; 
		virtual int getOptDirection() = 0;
		virtual void initiallization(Individual *ejInd) { }
		virtual void setParameters(vector<double> &parameters) { cerr << "Llamado setParameters en clase padre" << endl; exit(-1); }
};
