...
void ExecutionIsland::run(const Configuration &configuration, int seed){
	double finalScore = 0;
	...
	while(1){
		...
		if (tamBuffer < 3){
			delete [] buffer;
			buffer = new long double[3];
			tamBuffer = 3;
		}
		MPI_Recv(buffer, 3, MPI_LONG_DOUBLE, myMaster, ALG_TAG, MPI_COMM_WORLD, &status);
		numExecution++;
		// buffer[0] = Algoritmo || ALG_FINISH
		// buffer[1] = IdConf
		int newAlgId = (int)buffer[0];
		int newConfigId = (int)buffer[1];
		int stopCrit = (int)buffer[2];
		...
		if ((newAlgId != lastAlg) || (newConfigId != lastConf)){ //configuracion nueva
			map<string,double> shared;
			map<string,void*> sharedPointers;
			bool assigned = false;
			if (algCode){
				shared = algCode->getSharedParameters();
				sharedPointers = algCode->getSharedParametersPointers();
				assigned = true;
			}
			delete algCode;
			...
			if (assigned){
				algCode->setSharedParameters(shared);
				algCode->setSharedParametersPointers(sharedPointers);
			}
			...
			algCode->setStoppingCriterion(algorithms[newAlgId].critStop, stopCrit);
			...
			if (restartInfo){
				algCode->setRestartInfo(restartInfo);
				delete restartInfo;
			}
		}
		...
		for (int i = 0; i < newInitPopulation.size(); i++){
			if ((origin[i] == GLOBAL) || (lastAlg == newAlgId)){//ya esta convertido
				...
				if (newInitPopulation[i] != newInitPopulationConverted[i]){
		...
		if (scoreAlgorithm->getType() == LOCAL_SCORE_ALGORITHM){
			if (((LocalScoreAlgorithm *)(scoreAlgorithm))->getScoreType() == SCORE_WITH_NONDOM){
				...
				initScore = ((LocalScoreAlgorithm *)(scoreAlgorithm))->score(algCode->population);
			}
		}
					...
					if (flag){
						...
						if (scoreAlgorithm->getType() == LOCAL_SCORE_ALGORITHM){
							...
							if (((LocalScoreAlgorithm *)(scoreAlgorithm))->getScoreType() == SCORE_WITH_NONDOM){
								...
								cerr << "Error : el codigo no esta preparado para usar SCORE_WITH_POPOPULATION y migracion" << endl;
								exit(-1);
							}
							double after = 0;
		...
		if (scoreAlgorithm->getType() == LOCAL_SCORE_ALGORITHM){
			if ( (((LocalScoreAlgorithm *)(scoreAlgorithm))->getScoreType() == SCORE_WITH_NONDOM) && ((algorithms[newAlgId].solutionSource == SOURCE_ARCHIVE)) ) {
				finalScore = ((LocalScoreAlgorithm *)(scoreAlgorithm))->score(algCode->getLocalSolution());
			} else if ( (((LocalScoreAlgorithm *)(scoreAlgorithm))->getScoreType() == SCORE_WITH_NONDOM) && ((algorithms[newAlgId].solutionSource == SOURCE_ALGORITHM)) ) {
				finalScore = ((LocalScoreAlgorithm *)(scoreAlgorithm))->score(algSolution);
			} else {//SCORE_WITH_POP
				finalScore = ((LocalScoreAlgorithm *)(scoreAlgorithm))->score(algCode->population);
			}
		}
		...
		if (scoreAlgorithm->getType() == LOCAL_SCORE_ALGORITHM){
			...
			newscores[2] = algCode->getNumImprovements();
			migrationImprovement = 0;
			MPI_Send(&newscores, 3, MPI_LONG_DOUBLE, myMaster, LOCAL_SCORE, MPI_COMM_WORLD);
		}
		evaluations = algCode->getPerformedEvaluations();
		evaluations -= lastEvaluationsSend;
		MPI_Send(&evaluations, 1, MPI_INT, myMaster, NUM_EVALUATIONS, MPI_COMM_WORLD);//send the number of evaluations
