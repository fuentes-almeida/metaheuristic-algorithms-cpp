...
int main (int argc, char *argv[]) {
	...
	if ((argc != 4) && (argc != 5)){
		if (myId == 0)
			cout << "Uso correcto: " << argv[0] << " outputFile configFile randomSeed [printPeriod]" << endl;
	...
	CoordinatorIsland coordinator(argv[1], ((argc==5)?atoi(argv[4]):0), baseDirectory, atoi(argv[3]) + 1);

	// Tiempo inicial
	gettimeofday(&t1, NULL);

	// Ejecución del Team Algorithm
	coordinator.run (argv[2], atoi(argv[3]) + 10000);
