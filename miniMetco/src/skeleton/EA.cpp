...
void EA::setStoppingCriterion(const int critStop, const double critStopValue) {
	...
	if (critStop == getTypeStoppingCriterion("TIME")) {
		...
		startTime2 = ((double) (actualTime.tv_sec) * 1.0e6 + (double) (actualTime.tv_usec))/1.0e6;
...
double EA::getCompletedRatio(){
	if (critStop == getTypeStoppingCriterion("TIME")) {
		...
		if (at >= time){
			...
			return (at - startTime2) / (critStopValue);
		}
	} else if (critStop == getTypeStoppingCriterion("EVALUATIONS")) {
		if (evaluationActual > evaluations){
			return 1;
		} else {
			return (double)(evaluationActual) / evaluations;
		}
	} else if (critStop == getTypeStoppingCriterion("QUALITY")){
		cerr << "No implementado!!!" << endl;
		exit(-1);
	} else {
		cout << "\tError: Stopping criterion nondefined" << endl << flush; 
		return (true);
...
void EA::run() {
	...
	cout << "Va a llamar a fillPop" << endl;
	fillPopWithNewIndsAndEvaluate();
	cout << "Vuelve de fillPop" << endl;
	do {
		checkPrint();
		runGenerationInc();
	} while (!hasFinished());
