...
public:
	...
	virtual void localSearch() { cerr << "Local search llamada pero no implementada" << endl; exit(-1); }
	...
	virtual void getGenesAndAlleles(vector<int> &genes, vector<int> &options) const { cout << "Llama a getGenesAndAlleles" << endl; exit(-1);};
