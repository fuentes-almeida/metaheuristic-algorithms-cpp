...
public:
	// Constructor
	CoordinatorIsland (const string &outputFileName, const int printPeriod, const string &baseDirectory, int seed); 

	// Destructor
	~CoordinatorIsland (void);

	// Arrancar la ejecución paralela
	void run  (const char *file, int seed);
...
private:
	...
	vector<int> mySlaves;
	int performedSelections;
	int numberOfConfigurations;
	int evaluationsPerBlock;
	map< int, map<int, double> > stopCriterions;
	vector<int> orderedAlgs, orderedConfigs;
	vector<int> orderedConfigurations;
