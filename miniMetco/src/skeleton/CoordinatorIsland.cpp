...
// Constructor
CoordinatorIsland::CoordinatorIsland (const string &outputFileName, const int printPeriod, const string &baseDirectory, int seed){
	//MPI Initialization
	...
	if (seed == -1){
		unsigned int seed2 = (unsigned int)t1.tv_usec + t1.tv_sec;
		seed2 *= (myId + 1);
		srand(seed2);
	} else {//OJO Todos iguales, solo validos para los casos con una isla
		initFixedRandomSeed(seed);
	}
}
...
void CoordinatorIsland::init (const char *confFile) {
	...
	if (configuration.getScoreAlgorithm()->getType() == LOCAL_SCORE_ALGORITHM){
		if (((LocalScoreAlgorithm *)(configuration.getScoreAlgorithm()))->getScoreType() == SCORE_WITH_NONDOM){
			((LocalScoreAlgorithm *)(configuration.getScoreAlgorithm()))->score(moTemp); 
		} else {
			vector<Individual *> tmp;
			tmp.push_back(getEjInd());
			((LocalScoreAlgorithm *)(configuration.getScoreAlgorithm()))->score(&tmp); 
		}
	} else {
	...
	numberOfConfigurations = 0;
	evaluationsPerBlock = 0;
	for (int i = 0; i < configuration.getTeamAlgorithms().size(); i++){
		configInfo.push_back(configuration.getTeamAlgorithms()[i].configs.size());
		evaluationsPerBlock += (configuration.getTeamAlgorithms()[i]).valueSC * configuration.getTeamAlgorithms()[i].configs.size();
		for (int j = 0; j < configuration.getTeamAlgorithms()[i].configs.size(); j++){
			orderedAlgs.push_back(i);
			orderedConfigs.push_back(j);
			numberOfConfigurations++;
			stopCriterions[i][j] = configuration.getTeamAlgorithms()[i].valueSC;
		}
	}
...
void CoordinatorIsland::initAlgorithm (const int indexAlg, const int actConf, const int idIsland){
  ...
  // Se almacena que algoritmo es ejecutado por la isla idIsland
	...
	long double *buffer = new long double[3];
	buffer[0] = indexAlg;
	buffer[1] = actConf;
	double sc = stopCriterions[indexAlg][actConf];
	if (configuration.getExecutionModel()[actualModel].stopCrit == GLOBAL_EVALUATIONS){
		sc = min((int)stopCriterions[indexAlg][actConf], (int)valueStopCrit - evaluations);
	}
	//cout << "Haria: " << (int)stopCriterions[indexAlg][actConf] << endl;
	//cout << "Manda como stop: " << sc << endl;
	//cout << "Evaluations es :" << evaluations << endl;
	buffer[2] = sc;
	MPI_Send(buffer, 3, MPI_LONG_DOUBLE, idIsland, ALG_TAG, MPI_COMM_WORLD);
	delete buffer;
...
long double CoordinatorIsland::scoreConfiguration (const int indexAlg, const int config, const double rndValue, const int idle){
	if (configuration.getScoreAlgorithm()->getType() == LOCAL_SCORE_ALGORITHM){
		return ((LocalScoreAlgorithm *)configuration.getScoreAlgorithm())->estimateScore(lastScore, indexAlg, config, idle, rndValue, numberOfConfigurations);
...
void CoordinatorIsland::selectionIsland (int &indexAlg, int &config, const int idle){
	...
	//cout << "Entra en selection" << endl << flush;
	if (configuration.getExecutionModel()[actualModel].typeSelection == Configuration::getTypeSelection("SHARE_EVALUATIONS")) {
		//cout << "Va a serleccionar" << endl;
		double rndValue = (double)(rand()) / RAND_MAX;
		if ((performedSelections % numberOfConfigurations) == 0){//Update stop criterions
			double sumRatios = 0;
			for (unsigned int i = 0; i < configuration.getTeamAlgorithms().size(); i++) {
				for (unsigned int j = 0; j < configuration.getTeamAlgorithms()[i].configs.size(); j++) {
					double ratio = scoreConfiguration(i, j, rndValue, idle);
					sumRatios += ratio;
					stopCriterions[i][j] = ratio * evaluationsPerBlock;
					//cout << "Ev. per block: " << evaluationsPerBlock << endl;
				}
			}
			if ((sumRatios > 1.0001) || (sumRatios < 0.9999)){
				cout << "Error interno. Los ratios no suman 1" << endl << flush;
				exit(-1);
			}
		}
		indexAlg = orderedAlgs[performedSelections % numberOfConfigurations];
		config = orderedConfigs[performedSelections % numberOfConfigurations];
		performedSelections++;
		return;
	}
...
int CoordinatorIsland::recvLocalSolution (){
	...
	if (localFinish){
		...
		if (configuration.getScoreAlgorithm()->getType() == LOCAL_SCORE_ALGORITHM){
			...
			long double newscores[3];
			MPI_Status status;
			MPI_Recv(newscores, 3, MPI_LONG_DOUBLE, src, LOCAL_SCORE, MPI_COMM_WORLD, &status);
			((LocalScoreAlgorithm *)configuration.getScoreAlgorithm())->updateScore(newscores[0], newscores[1], newscores[2], indexAlg, config, src % numberOfProcessesPerExecution);
...
void CoordinatorIsland::run (const char *file, int seed) {
	init (file);
